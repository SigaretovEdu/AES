...
int main() {
	...
	std::ofstream Cipher("../cipher.txt");
...
void AES::InvMixColumns(std::vector<std::vector<unsigned char>>& state) {
	...
	for(size_t pos = 0; pos < this->NB; pos++) {
		temp[0][pos] =
			GF[14][state[0][pos]] ^ GF[11][state[1][pos]] ^ GF[13][state[2][pos]] ^ GF[9][state[3][pos]];
		temp[1][pos] =
			GF[9][state[0][pos]] ^ GF[14][state[1][pos]] ^ GF[11][state[2][pos]] ^ GF[13][state[3][pos]];
		temp[2][pos] =
			GF[13][state[0][pos]] ^ GF[9][state[1][pos]] ^ GF[14][state[2][pos]] ^ GF[11][state[3][pos]];
		temp[3][pos] =
			GF[11][state[0][pos]] ^ GF[13][state[1][pos]] ^ GF[9][state[2][pos]] ^ GF[14][state[3][pos]];
	}
	state = temp;
}
